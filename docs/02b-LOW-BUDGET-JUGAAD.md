...
| 10 | VL53L0X ToF | 250–600 | **HC-SR04 ultrasonic (₹60)** — or SKIP it in v1: the firmware already detects stalls ("bump") and the escape maneuver un-wedges the rover | 0–60 | Ultrasonic needs a small firmware patch (~10 lines of trigger/echo pulse timing — any coding assistant writes it in a minute). Skipping it means the map discovers rocks more slowly; slow speed keeps you safe. |
